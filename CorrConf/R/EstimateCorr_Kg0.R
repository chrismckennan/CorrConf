...
Optimize.Theta.multB <- function(Y, maxK, B, Cov=NULL, A=NULL, c=NULL, D.ker=NULL, Var.0=NULL, tol.rho=1e-3, max.iter.rho=10, svd.method="fast") {
  ...
  for (k in 1:maxK) {
    ...
    if (svd.method == "fast") {
      out$C[[k+1]] <- sqrt(n) * svd( sqrt.V %*% cbind(irlba(A=(Y %*% sqrt.Vinv) / sqrt(Delta.0), nv = k, tol = 1/sqrt(n) * 1e-4)$v) )$u
    } else {
      out$C[[k+1]] <- sqrt(n) * svd( sqrt.V %*% cbind(svd((Y %*% sqrt.Vinv) / sqrt(Delta.0), nv = k)$v) )$u
    }
    out$Rho[k+1,] <- Rho.0
  }
  return(out)
...
seq.PCA.multB <- function(Y, B, K, Rho.0, Delta.0=NULL, A=NULL, c=NULL, D.ker=NULL, svd.method="fast", max.iter=10, tol.rho=1e-3) {
  ...
  for (i in 1:max.iter) {
    ...
    if (svd.method == "fast") {
      s.0 <- irlba(A=(Y %*% sqrt.Vinv) / sqrt(Delta.0), nv = K, tol = 1/sqrt(n) * 1e-4)
    } else {
      s.0 <- svd((Y %*% sqrt.Vinv) / sqrt(Delta.0))
    }
